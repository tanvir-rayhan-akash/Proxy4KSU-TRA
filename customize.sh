...
installModule() {
    ...
    if [ -f /sdcard/proxy4ksu.setup ]; then
        use_param=true
        param_core=$(sed -n 1p /sdcard/proxy4ksu.setup)
        param_keep=$(sed -n 2p /sdcard/proxy4ksu.setup)
        param_uitype=$(sed -n 3p /sdcard/proxy4ksu.setup)
    fi
    ...
    unzip -j -o "${ZIPFILE}" 'proxy4ksu_service.sh' -d /data/adb/service.d >&2
    ...
    set_perm /data/adb/service.d/proxy4ksu_service.sh 0 0 0755
